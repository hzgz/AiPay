...
import {
  ...
  readScopedThemeValue,
  scopedSettingStorage,
  writeScopedThemeValue
} from '@/utils/storage/settingScope'
...
export const useSettingStore = defineStore(
  ...
  () => {
    ...
    const applyScopedThemeFallback = (scope: string, hasPersistedPayload: boolean) => {
      const scopedTheme = readScopedThemeValue(scope)
      if (scopedTheme !== SystemThemeEnum.LIGHT && scopedTheme !== SystemThemeEnum.DARK) {
        return
      }

      systemThemeType.value = scopedTheme

      if (!hasPersistedPayload) {
        systemThemeMode.value = scopedTheme
      }
    }

    const syncPersistedScope = (path?: string, force: boolean = false) => {
      const nextScope = resolveConsoleSettingStorageScope(path)
      if (!force && activeScope.value === nextScope) {
        return false
      }
      ...
      if (!raw) {
        applyPersistedState()
        applyScopedThemeFallback(nextScope, false)
        return true
      }

      try {
        applyPersistedState(JSON.parse(raw) as Partial<Record<PersistedStateKey, unknown>>)
        applyScopedThemeFallback(nextScope, true)
      } catch (error) {
