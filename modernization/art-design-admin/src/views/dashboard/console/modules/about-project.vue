<template>
  <div class="art-card p-5 flex-b mb-5 max-sm:mb-4">
    <div>
      ...
      <p class="text-g-700 mt-1">{{ systemName }} 是面向商户、通道与订单运营的一体化支付管理后台</p>
      <p class="text-g-700 mt-1">当前已统一管理员后台、商户后台与支付主链路，便于日常运营、排障与上线前巡检。</p>
...
<script setup lang="ts">
  ...
  const buildHashUrl = (path: string) => new URL(`#${path}`, window.location.href).toString()

  const linkList = [
    { label: '支付插件', url: buildHashUrl('/payments/plugins') },
    { label: '系统配置', url: buildHashUrl('/system/config') },
    { label: '进程管理', url: buildHashUrl('/system/processes') },
    { label: '角色权限', url: buildHashUrl('/system/role') }
  ]
