...
<script lang="ts" setup>
  ...
  function flattenMenuMap(items: AppRouteRecord[]): Map<string, AppRouteRecord> {
    const map = new Map<string, AppRouteRecord>()

    const collect = (item: AppRouteRecord) => {
      if (item.meta?.isHide) return
      ...
      const key = item.path || String(item.meta.link || '')
      if (key.trim()) {
        map.set(key, { ...item, children: undefined })
      }
    }
  ...
  function sanitizeHistory() {
    if (!Array.isArray(menuList.value) || menuList.value.length === 0) {
      return
    }

    const menuMap = flattenMenuMap(menuList.value)
    const sanitizedHistory = historyResult.value.reduce((acc: AppRouteRecord[], item) => {
      const key = item.path || String(item.meta.link || '')
      ...
      const canonicalItem = menuMap.get(normalizedKey)
      if (canonicalItem) {
        acc.push(canonicalItem)
      }
      return acc
    }, [])

    const historyChanged =
      sanitizedHistory.length !== historyResult.value.length ||
      sanitizedHistory.some((item, index) => {
        const currentItem = historyResult.value[index]
        const currentKey = currentItem?.path || String(currentItem?.meta?.link || '')
        const currentQuery = JSON.stringify(
          ((currentItem as AppRouteRecord & { query?: unknown })?.query ?? null) as unknown
        )
        const currentParams = JSON.stringify(
          ((currentItem as AppRouteRecord & { params?: unknown })?.params ?? null) as unknown
        )
        const nextKey = item.path || String(item.meta.link || '')

        return (
          nextKey !== currentKey ||
          formatMenuTitle(item.meta.title) !== formatMenuTitle(currentItem?.meta?.title) ||
          currentQuery !== 'null' ||
          currentParams !== 'null'
        )
      })
    ...
    if (historyChanged) {
      userStore.setSearchHistory(sanitizedHistory)
      historyHIndex.value = Math.min(historyHIndex.value, Math.max(sanitizedHistory.length - 1, 0))
    }
  }
