...
import { fetchGetPaymentPlugins } from '@/api/systemManage'
...
export function usePaymentAccountFormState({
  ...
  function buildCreatePayload(): Api.Payments.AccountCreatePayload | null {
    ...
    if (
      shouldShowAccountIdentifierField(createForm.code, createForm.qr_type) &&
      !createForm.identifier
    ) {
      ElMessage.warning(
        `请输入${displayAccountFieldLabel(activeCreateMeta.value.identifierLabel)}`
      )
      return null
    }
    ...
    if (
      ...
      ElMessage.warning('支付宝软件版图片模式必须填写二维码内容。')
      return null
    }
    ...
    if (createForm.code === 'usdt' && createForm.remark && !/^[1-9]\d*$/.test(createForm.remark)) {
      ElMessage.warning('USDT 订单时长必须填写正整数秒')
      return null
    }
    ...
    return {
      ...
      identifier: shouldShowAccountIdentifierField(createForm.code, createForm.qr_type)
        ? createForm.identifier
        : '',
  ...
  function buildCredentialPayload(
    ...
    if (shouldShowAccountIdentifierField(code, credentialForm.qr_type) && !credentialForm.identifier) {
      ElMessage.warning(
        `请输入${displayAccountFieldLabel(meta.identifierLabel)}`
      )
      return null
    }
    ...
    if (code === 'usdt' && credentialForm.remark && !/^[1-9]\d*$/.test(credentialForm.remark)) {
      ElMessage.warning('USDT 订单时长必须填写正整数秒')
      return null
    }
    ...
    return {
      identifier: shouldShowAccountIdentifierField(code, credentialForm.qr_type)
