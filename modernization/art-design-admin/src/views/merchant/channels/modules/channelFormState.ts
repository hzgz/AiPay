...
export function buildMerchantChannelCreatePayload(
  ...
  if (shouldShowAccountIdentifierField(form.code, form.qr_type) && !form.identifier) {
    ElMessage.warning(
      `请输入${displayAccountFieldLabel(activeCreateMeta.identifierLabel)}`
    )
    return null
  }
  ...
  if (
    activeCreateMeta.supportsPid &&
    form.code !== 'alipay_official' &&
    form.code !== 'alipay_software' &&
    !form.pid
  ) {
  ...
  if (form.code === 'usdt' && form.remark && !/^[1-9]\d*$/.test(form.remark)) {
  ...
  return {
    ...
    code: form.code,
    identifier: shouldShowAccountIdentifierField(form.code, form.qr_type) ? form.identifier : '',
...
export function buildMerchantChannelCredentialPayload(
  ...
  if (shouldShowAccountIdentifierField(code, form.qr_type) && !form.identifier) {
    ElMessage.warning(
  ...
  if (code === 'usdt' && form.remark && !/^[1-9]\d*$/.test(form.remark)) {
    ElMessage.warning('USDT 订单时长必须填写正整数秒')
    return null
  }
  ...
  return {
    identifier: shouldShowAccountIdentifierField(code, form.qr_type) ? form.identifier : '',
