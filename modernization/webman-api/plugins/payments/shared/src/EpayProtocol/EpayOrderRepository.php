...
class EpayOrderRepository
{
    ...
    public function create(array $merchant, array $payload, ?array $paylist, array $basicSettings, string $clientIp = ''): array
    {
        ...
        $draft['id'] = (int)Db::table(BusinessTable::order())->insertGetId($insert);
        unset($draft['migration_state']);
    ...
    public function settlePaidOrder(array $order, array $merchant, array $payload): array
    {
        return Db::transaction(function () use ($order, $merchant, $payload): array {
            ...
            $settled = array_merge($current, [
                'status' => $alreadyPaid ? (int)($current['status'] ?? 0) : 1,
                'end_time' => $alreadyPaid ? (string)($current['end_time'] ?? '') : $endTime,
                'update_time' => $alreadyPaid ? (string)($current['update_time'] ?? '') : $updateTime,
                'alipay_order_no' => !$alreadyPaid
                    ? ($transactionNo ?: (string)($current['alipay_order_no'] ?? ''))
                    : (string)($current['alipay_order_no'] ?? ''),
            ]);
