...
<script setup lang="ts">
  ...
  import { displayAdminRoleOptionLabel } from './adminFormState'
  import type { AdminPermissionFormState, AdminRoleFormState } from './adminFormState'
...
<style scoped lang="scss">
  .permission-editor {
    ...
    gap: 16px;
    --admin-access-scroll-border: var(--el-border-color-lighter);
    --admin-access-scroll-bg: rgb(248 250 252 / 0.55);
  }

  :global(html.dark .permission-editor ){
    --admin-access-scroll-border: rgb(71 85 105 / 0.42);
    --admin-access-scroll-bg: rgb(15 23 42 / 0.72);
  }

  .permission-scroll {
    border: 1px solid var(--admin-access-scroll-border);
    border-radius: 14px;
    padding: 12px;
    background: var(--admin-access-scroll-bg);
  }
