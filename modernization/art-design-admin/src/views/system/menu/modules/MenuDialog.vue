...
<script setup lang="ts">
  ...
  import type { FormItem } from '@/components/core/forms/artForm/index.vue'
  import ArtForm from '@/components/core/forms/artForm/index.vue'
  ...
  const rules = reactive<FormRules>({
    ...
    label: [{ required: true, message: '请输入菜单标识', trigger: 'blur' }],
    authName: [{ required: true, message: '请输入权限名称', trigger: 'blur' }],
    authLabel: [{ required: true, message: '请输入操作标识', trigger: 'blur' }]
  })
  ...
  const formItems = computed<FormItem[]>(() => {
    ...
    if (form.menuType === 'menu') {
      return [
        ...
        { label: '菜单标识', key: 'label', type: 'input', props: { placeholder: '如：systemUser' } },
        {
          label: createLabelTooltip(
            '组件路径',
        ...
        { label: '图标', key: 'icon', type: 'input', props: { placeholder: '如：ri:user-line' } },
        {
          label: createLabelTooltip(
            '角色权限',
            '前端权限模式下配置角色标识（如 R_SUPER、R_ADMIN）\n后端权限模式下无需配置'
          ),
          key: 'roles',
          type: 'inputtag',
        ...
        },
        {
          ...
          key: 'link',
          type: 'input',
          props: { placeholder: '例如：https://你的站点地址' }
        },
        {
          label: '文本徽章',
          key: 'showTextBadge',
