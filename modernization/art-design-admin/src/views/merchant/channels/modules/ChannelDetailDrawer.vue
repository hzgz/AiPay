      ...
      <template v-if="activeAccount">
          ...
          <div class="detail-hero-copy">
            ...
            <span>{{
              displayAccountTypeSummary(
                activeAccount.type_text || activeAccount.type_label,
                activeAccount.type
              )
            }}</span>
          </div>
        ...
        <section class="channel-form-section channel-form-section--actions">
          ...
          <div class="channel-detail-actions">
            <ElButton v-if="hasTestPayAuth" plain :disabled="detailLoading" @click="emit('test')">
              测试
            </ElButton>
...
<script setup lang="ts">
  ...
  function displayAccountQrType(account?: Partial<AccountItem> | null, fallback = '未配置') {
    return displayAccountFieldText(
      account?.qr_type_text || account?.qr_type_label || account?.qr_type,
      fallback
    )
  }
  ...
  function displayAccountMemo(account?: Partial<AccountItem> | null, fallback = '--') {
    return displayAccountFieldText(
      account?.memo_text || account?.memo_label || account?.memo,
      fallback
    )
  }
...
<style scoped lang="scss">
  .channel-shell-drawer {
    --merchant-drawer-panel-bg: var(--default-box-color);
    --merchant-drawer-shell-bg: var(--default-bg-color);
    --merchant-drawer-muted: #7b88a1;
    --merchant-drawer-heading: var(--art-gray-900);
    --merchant-drawer-border: var(--art-card-border);
    --merchant-drawer-shadow: 0 12px 32px rgb(15 23 42 / 5%);
    --merchant-drawer-soft-bg: rgb(148 163 184 / 8%);
    --merchant-drawer-soft-border: rgb(148 163 184 / 12%);
  }

  :global(html.dark .channel-shell-drawer ){
    --merchant-drawer-muted: #8fa0bd;
    --merchant-drawer-heading: #f8fafc;
    --merchant-drawer-shadow: 0 18px 38px rgb(2 6 23 / 28%);
    --merchant-drawer-soft-bg: rgb(148 163 184 / 10%);
    --merchant-drawer-soft-border: rgb(148 163 184 / 18%);
  }
  ...
  .channel-shell-drawer :deep(.el-drawer__header) {
    ...
    border-bottom: 1px solid var(--merchant-drawer-border);
    background: color-mix(in srgb, var(--merchant-drawer-panel-bg) 96%, transparent);
  }

  .channel-shell-drawer :deep(.el-drawer__title) {
    color: var(--merchant-drawer-heading);
    font-size: 16px;
    font-weight: 700;
  ...
  .channel-shell-drawer :deep(.el-drawer__body) {
    ...
    background:
      ...
      linear-gradient(
        180deg,
        color-mix(in srgb, var(--merchant-drawer-shell-bg) 92%, transparent),
        var(--merchant-drawer-panel-bg) 180px
      );
  }

  .channel-shell-drawer :deep(.el-drawer__headerbtn) {
    color: var(--merchant-drawer-muted);
  }

  .channel-shell-drawer :deep(.el-drawer__headerbtn:hover) {
    color: var(--merchant-drawer-heading);
  }

  .payment-account-detail :deep(.el-loading-mask) {
    background: color-mix(in srgb, var(--merchant-drawer-shell-bg) 86%, transparent);
  }
  ...
  .detail-hero--drawer {
    padding: 18px;
    border: 1px solid var(--merchant-drawer-border);
    border-radius: 20px;
    background:
      linear-gradient(
        135deg,
        color-mix(in srgb, var(--merchant-drawer-panel-bg) 98%, white 2%),
        color-mix(in srgb, var(--merchant-drawer-panel-bg) 88%, var(--merchant-drawer-shell-bg))
      ),
      linear-gradient(135deg, rgb(59 130 246 / 0.08), rgb(14 165 233 / 0.04));
    box-shadow: var(--merchant-drawer-shadow);
  }
  ...
  .channel-form-section {
    padding: 14px;
    border: 1px solid var(--merchant-drawer-soft-border);
    border-radius: 16px;
    background: color-mix(in srgb, var(--merchant-drawer-panel-bg) 94%, transparent);
  }
  ...
  .detail-item {
    ...
    background: color-mix(
      in srgb,
      var(--merchant-drawer-soft-bg) 82%,
      var(--merchant-drawer-panel-bg)
    );
  }
