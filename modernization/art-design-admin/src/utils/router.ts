...
export const setPageTitle = (to: RouteLocationNormalized): void => {
  const { title } = to.meta
  if (title) {
    const resolvedTitle = to.path === '/' && to.meta?.publicLanding ? '首页' : formatMenuTitle(String(title))
    const pageTitle = `${resolvedTitle} - ${resolveRouteAppTitle(to)}`
...
function resolveRouteAppTitle(route: RouteLocationNormalized): string {
  if (route.meta?.publicLanding) {
    return 'AiPay'
  }

  return resolveAppTitle(route.path)
}
